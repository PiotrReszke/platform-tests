...
class Topic(object):
    ...
    def __init__(self, node_name, broker_instance_name, app_name, topic_name):
        self.hdfs = Hdfs(node_name)
        self.name = topic_name
        self.path = os.path.join(self.CF_BROKER_INSTANCES_PATH, broker_instance_name, app_name, topic_name)

    def __repr__(self):
        return "{} (name={})".format(self.__class__.__name__, self.name)

    @classmethod
    def get_topics(cls, node_name, broker_instance_name, app_name):
        hdfs = Hdfs(node_name)
        path = os.path.join(cls.CF_BROKER_INSTANCES_PATH, broker_instance_name, app_name)
        topic_paths = hdfs.ls(path)
        topics = []
        for topic_path in topic_paths:
            topic_name = re.search("[^/]+$", topic_path).group(0)
            topics.append(cls(node_name, broker_instance_name, app_name, topic_name))
