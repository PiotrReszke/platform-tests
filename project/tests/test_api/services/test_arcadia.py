...
class ArcadiaTest(ApiTestCase):
    ...
    def test_0_create_new_dataset_and_import_it_to_arcadia(self):
        ...
        self.assertInList(self.test_org.name, db_list, "Organization not found in database list in arcadia")
        ...
        self.assertInList(self.dataset.title, table_list, "Dataset not found in table list in arcadia")
        self.step("Create new dataset in arcadia")
        arcadia_dataset = self.arcadia.create_dataset(self.test_org.name, self.dataset.title)
        self.assertInListWithRetry(arcadia_dataset, self.arcadia.get_dataset_list)
